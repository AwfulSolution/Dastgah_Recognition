...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--failed_list",
        default=os.path.join(ROOT, "logs", "wav_conversion_failed_files.txt"),
        help="Path to write failed source file paths",
    )
    p.add_argument(
        "--failed_details",
        default=os.path.join(ROOT, "logs", "wav_conversion_failed_details.log"),
        help="Path to write detailed conversion errors",
    )
    return p.parse_args()
...
def _convert_one(job: Tuple[str, str, int, bool, bool]) -> Tuple[str, str, str]:
...
def main() -> None:
    ...
    os.makedirs(os.path.dirname(args.failed_list), exist_ok=True)
    os.makedirs(os.path.dirname(args.failed_details), exist_ok=True)
    ...
    if args.num_workers <= 1:
        ...
        for job in iterator:
            status, src, details = _convert_one(job)
            if status == "converted":
                converted += 1
            elif status == "skipped":
                skipped += 1
                ...
                failed += 1
                failed_entries.append((src, details))
    else:
    ...
    with open(args.failed_list, "w", encoding="utf-8") as f:
        for src, _ in failed_entries:
            f.write(f"{src}\n")
    with open(args.failed_details, "w", encoding="utf-8") as f:
        for src, details in failed_entries:
            f.write(f"### {src}\n{details}\n\n")

    print(f"Done. converted={converted} skipped={skipped} failed={failed} total={len(worker_jobs)}")
    if failed > 0:
        print(f"Failed files list: {args.failed_list}")
        print(f"Failed details log: {args.failed_details}")
