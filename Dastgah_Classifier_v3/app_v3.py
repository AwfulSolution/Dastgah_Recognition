...
def load_bundle(model_path: str, cfg_path: str) -> Dict:
    ...
    labels = cfg["labels"]
    feat_cfg = MelodicFeatureConfig(**cfg["feature_config"])
...
def predict_file(bundle: Dict, file_path: str) -> Tuple[str, np.ndarray]:
    feat = extract_track_feature(file_path, bundle["feat_cfg"], "inference", 42, bundle["cache_dir"]).reshape(1, -1)
    model = bundle["model"]
...
st.set_page_config(page_title="Dastgah Classifier v3", layout="centered")
st.title("Dastgah Classifier v3")
st.caption("Phrase-aware melodic model with track-level tonic and note-event features.")
...
if mode == "Single model":
    ...
    if source == "Run directory":
        ...
        selected_bundle = load_bundle(os.path.join(selected_run, "model.joblib"), os.path.join(selected_run, "model_config.json"))
    else:
        if prod is None:
    ...
    if st.button("Classify", key="single_classify"):
        ...
        try:
            ...
            for display_name, tmp_path in temp_files:
                ...
                rows.append({"file": display_name, "prediction": pred_label, "confidence": float(np.max(probs))})
