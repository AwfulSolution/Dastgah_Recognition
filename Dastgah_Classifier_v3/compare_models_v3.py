...
def collect_runs(runs_dir: str) -> List[Dict]:
    ...
    for name in sorted(os.listdir(runs_dir)):
        ...
        if metrics is None:
            continue
        if not os.path.exists(os.path.join(run_dir, "model.joblib")):
            continue
        if not os.path.exists(os.path.join(run_dir, "model_config.json")):
            continue
...
def main() -> None:
    ...
    p.add_argument("--out", default=os.path.join(ROOT, "runs", "compare_models_v3.md"))
